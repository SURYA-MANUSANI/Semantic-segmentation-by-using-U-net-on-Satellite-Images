...
# Optionally evaluate
loss, accuracy, jacard_coef = model.evaluate(X_test, y_test , verbose=2)
...
def visualize_prediction(image, mask_true, mask_pred):
    ...
    axes[1].imshow(mask_true[:, :, 0])
    axes[1].set_title("Ground Truth")
    axes[2].imshow(np.argmax(mask_pred, axis=-1))
    axes[2].set_title("Predicted Mask")
    plt.show()

# --- Use the visualization on one test sample ---
index = 0
pred = model.predict(np.expand_dims(X_test[index], axis=0))
visualize_prediction(X_test[index], y_test[index], pred[0])
...
model.save("models/satellite_standard_unet_5epochs.hdf5")

#model.save('models/satellite_standard_unet')
############################################################
...
model = load_model("models/satellite_standard_unet_5epochs.hdf5",
